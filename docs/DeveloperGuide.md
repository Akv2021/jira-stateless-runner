...
- **When:** Before every push; auto-fires in pre-commit. Required for every milestone Definition-of-Done (refer to `/temp/ExternalRunner.md §9.4`).
...
- **When:** Every code change. **M1 gate requires 23 collected, all RED against the `NotImplementedError` stub**; **M2 gate requires 23 / 23 GREEN** (refer to `/temp/ExternalRunner.md §9.2` and `/temp/ImplementationTestMatrix.md`).
- **Green signal:** `23 passed in 0.Xs` (M2+) or `23 failed` (M1 RED baseline).
...
- **When:** Exactly once per cloned working tree (refer to `/temp/ImplementationRoadmap.md` M0.10). Idempotent — re-running is harmless.
